import { 
  ...
  SolverResult,
  FlowState 
} from "@/lib/hydraulics/open-channel/types";
import { calculateHydraulicProperties, getMaximumDepth } from "@/lib/hydraulics/open-channel/geometry";
import { solveRobust, findBracket } from "@/lib/hydraulics/open-channel/solver";
...
export function calculateChannelHydraulics(inputs: ChannelInputs): CalculationResult<HydraulicResults> {
  try {
    ...
    const results: HydraulicResults = {
      ...
      froudeNumber: round(froudeNumber, 4),
      flowState,
      reynoldsNumber: round(reynoldsNumber, 0),
      specificEnergy: round(specificEnergy, 4),
      criticalDepth: round(criticalDepth, 4),
      criticalSlope: round(criticalSlope, 6),
      hydraulicProperties: {
        area: round(props.area, 4),
        wettedPerimeter: round(props.wettedPerimeter, 4),
        hydraulicRadius: round(props.hydraulicRadius, 4),
        topWidth: round(props.topWidth, 4),
        hydraulicDepth: round(props.hydraulicDepth, 4)
      },
      freeboard: {
        lining: round(freeboard.lining, 4),
        bank: round(freeboard.bank, 4),
        controlling: round(Math.max(freeboard.lining, freeboard.bank), 4)
      },
      geometry: {
        totalDepth: round(normalDepth + Math.max(freeboard.lining, freeboard.bank), 4),
        topWidth: round(props.topWidth, 4)
      }
    };
